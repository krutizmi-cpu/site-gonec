...
export async function submitLeadAction(
  ...
  if (!webhookUrl) {
    return {
      success: false,
      message:
        "Канал приёма заявок временно обновляется. Пока удобнее связаться по телефону или e-mail из раздела контактов.",
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      message:
        "Не удалось передать заявку. Попробуйте ещё раз чуть позже или временно используйте прямые контакты из блока «Контакты».",
    };
  }
}
