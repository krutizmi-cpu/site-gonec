...
export async function submitLeadAction(
  ...
  if (!webhookUrl) {
    return {
      success: false,
      message:
        "Заявку через форму временно не принимаем. Пока удобнее связаться с нами по телефону или e-mail из раздела контактов.",
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      message:
        "Не удалось отправить заявку. Попробуйте ещё раз чуть позже или свяжитесь с нами напрямую через контакты.",
    };
  }
}
